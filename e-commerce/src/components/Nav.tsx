  ...
  return (
    <nav className="flex flex-wrap justify-between items-center z-10 relative">
      ...
      <button
        ...
        className="lg:hidden hover:bg-gray-100 focus:ring-2 focus:ring-gray-200 p-2 rounded-lg cursor-pointer"
      >
      ...
      >
        {" "}
        {/* Full width on small screens, auto width on large screens */}
        <ul className="lg:space-x-8 flex flex-col lg:flex-row lg:bg-transparent lg:border-none bg-gray-50 text-lg border border-gray-100 rounded-lg p-4">
          ...
          {ROUTES.map((route, i) => (
            <li
              className={`px-3 py-2 cursor-pointer rounded lg:hover:bg-transparent lg:hover:text-blue-500 ${i === 0 ? "bg-purple-400 text-white lg:bg-transparent lg:text-purple-400" : "hover:bg-gray-100"} ${(i == 3 || i == 4) && "lg:text-white"}`}
              key={route}
            >
              {route}
            </li>
          ))}{" "}
          {/*A unique key is required for each list item */}
        </ul>
      </div>
      {/* Shopping Bag */}
      <div
      ...
      >
        {" "}
        {/* Fixed position for the shopping bag icon, at the bottom left of the screen */}
        <div className="rounded-full bg-white shadow-md flex-center h-12 w-12 cursor-pointer">
